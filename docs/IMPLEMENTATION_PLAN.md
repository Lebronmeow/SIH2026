...
| 9 | **Rameswaram demo pack** | ⬜ | `scripts/fetch_demo_data.py`: real ERDDAP subsets (SST/chl/waves/currents) → `data/demo/rams/{ocean,weather}`, VLIZ IMBL (`line_id=1306`+`1311`) + attribution, manifest.json with retrieval dates, synthetic-labeled MPA + geometry fallback so the pack is self-contained |
